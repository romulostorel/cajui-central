...
// Polls until a final answer, or until the server reports the command undelivered.
// onPending is called once when the receiver accepted and waits for the transmitter.
export async function awaitAnswer(record, { onPending, interval = 1000 } = {}) {
  ...
  for (;;) {
    ...
    if (!response.ok) throw new Error(t("commands.errors.status"));
